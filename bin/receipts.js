...
// discipline. Never clobbers a hand-written AGENTS.md: the receipts block is delimited, so an
// existing file keeps its content and only the block between the markers is replaced.
...
function writeAgentsMd(dir) {
  const src = path.join(__dirname, "..", "adapters", "AGENTS.md");
  ...
  try { fs.writeFileSync(out, next); return { path: out, verb }; } catch { return null; }
}
...
async function init(opts) {
  ...
  // The same discipline, in the file a non-Claude agent reads. Opt out with --no-agents.
  if (!opts["no-agents"]) {
    const w = writeAgentsMd(dir);
    if (w) console.error(`\n${w.verb} ${w.path} - the agent-agnostic adapter (Codex, Cursor, any AGENTS.md reader)`);
  }
