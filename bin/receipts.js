 ...
 * receipts.config.json plus the agent-agnostic adapter (a short AGENTS.md block, the
 * full gates in .receipts/gates.md), and on --scaffold adds a project loop skill from
...
Usage:
  receipts init [options]      Detect this project, confirm, write receipts.config.json
                               (+ an AGENTS.md block and .receipts/gates.md; see --scaffold)
...
Options:
  ...
  --scaffold     Scaffold a <repo>-fix-loop skill when the project has no loop skill of
                 its own (init; off by default - the bundled gates skill drives the loop)
  --no-scaffold  Never scaffold one; skips the interactive offer too (init)
  --no-agents    Do not write AGENTS.md or .receipts/gates.md (init)
...
function buildConfig(d, a) {
  const cfg = {
    ...
    agent: {
      ...
      // so an upgrade can SAY "your config predates this" instead of the user being expected
      // to remember to re-check every repo after every update.
      receipts_version: OWN_VERSION,
    },
  };
...
// The CLI's own version, stamped into every config it writes. Without it nothing can say
// "this config predates the tool you are running" - staleness has to be guessed from which
// fields happen to be missing, which needs a new hand-written rule for every future field.
const OWN_VERSION = (() => {
  try { return require(path.join(__dirname, "..", "package.json")).version; } catch { return null; }
})();

// The Codex/agent-agnostic adapter, written INTO the repo so a non-Claude agent gets the same
// discipline. Two files: AGENTS.md gets a SHORT block (the rule, the ladder, the tripwire
// escapes, a pointer) and .receipts/gates.md gets the full adapter. Pasting all 23 KB into
// AGENTS.md drowned the house rules around it, and a rules file nobody reads enforces nothing.
// Never clobbers a hand-written AGENTS.md: the receipts block is delimited, so an existing
// file keeps its content and only the block between the markers is replaced. The block text
// lives here, not in a plugin file: the npm package ships bin/ + adapters/AGENTS.md only.
const AGENTS_BEGIN = "<!-- BEGIN receipts gates (generated by `receipts init` - edits inside are overwritten) -->";
const AGENTS_END = "<!-- END receipts gates -->";
const GATES_MD_HEADER = "<!-- GENERATED by `receipts init` from the receipts package's adapters/AGENTS.md - overwritten on every run, do not hand-edit -->";
const AGENTS_BLOCK = `
...
function writeAgentsMd(dir) {
  const body = readText(path.join(__dirname, "..", "adapters", "AGENTS.md"));
  if (!body) return null;
  // The full adapter is generated output, so it is simply overwritten on every run.
  const full = path.join(dir, ".receipts", "gates.md");
  try { fs.mkdirSync(path.dirname(full), { recursive: true }); fs.writeFileSync(full, `${GATES_MD_HEADER}\n${body}`); } catch { return null; }
  const block = `${AGENTS_BEGIN}\n${AGENTS_BLOCK.trim()}\n${AGENTS_END}\n`;
  const out = path.join(dir, "AGENTS.md");
  const existing = readText(out);
  let next, verb;
  if (!existing) { next = block; verb = "wrote"; }
  else if (existing.includes(AGENTS_BEGIN) && existing.includes(AGENTS_END)) {
    const re = new RegExp(`${escapeRe(AGENTS_BEGIN)}[\\s\\S]*?${escapeRe(AGENTS_END)}\\n?`);
    next = existing.replace(re, block); verb = "refreshed the receipts block in";
  } else {
    next = existing.replace(/\s*$/, "\n\n") + block; verb = "appended the receipts block to";
  }
  try { if (next !== existing) fs.writeFileSync(out, next); return { path: out, verb, full }; } catch { return null; }
}
...
async function init(opts) {
  ...
  if (!opts.yes) {
    ...
    try {
      ...
      // Offer to scaffold one if the project has no loop skill of its own. Default no: the
      // bundled `gates` already drives the loop, and a twin that repeated its trigger made
      // every bug a coin flip between the two.
      const hasProjectLoop = a.loop_skills.some((s) => s !== "gates");
      if (!hasProjectLoop && !opts["no-scaffold"]) {
        const yn = await ask(rl, `No project loop skill found. Scaffold one (${d.repo_name}-fix-loop) from the template?`, opts.scaffold ? "Y" : "N");
    ...
    // --yes: register the shipped loop + any detected project loops; scaffold only on --scaffold.
  ...
  if (!opts["no-agents"]) {
    const w = writeAgentsMd(dir);
    if (w) console.error(`\n${w.verb} ${w.path} (short receipts block) and wrote ${w.full} (the full gates G0-G19) - the agent-agnostic adapter (Codex, Cursor, any AGENTS.md reader)`);
  }
...
function doctor(opts) {
  ...
  // The direct signal, when present: which version wrote this file. Beats inferring staleness
  // from which fields happen to be missing (a rule that needs rewriting for every new field).
  if (OWN_VERSION && agent.receipts_version && agent.receipts_version !== OWN_VERSION)
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (x === "--scaffold") o.scaffold = true;
    else if (x === "--no-scaffold") o["no-scaffold"] = true;
    else if (x === "--no-agents") o["no-agents"] = true;
