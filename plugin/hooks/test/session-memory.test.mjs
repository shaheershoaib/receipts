...
// Read from the manifest, never hardcoded: a literal here breaks the suite on every
// version bump, which is exactly what happened when it was first written this way.
...
test("injects prior attempts for the current repo (config present + matching entries)", () => {
  const sb = sandbox({
    config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app" } },
    entries: [
...
test("total injected context is capped small (loads into every session)", () => {
  ...
  const sb = sandbox({ config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app" } }, entries: many });
  const text = injectedText(runHook(sb));
...
test("empty store -> silent", () => {
  const sb = sandbox({ config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app" } }, entries: "" });
  assert.equal(runHook(sb), null);
});

test("missing store file -> silent", () => {
  const sb = sandbox({ config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app" } } }); // entries omitted -> no file
  assert.equal(runHook(sb), null);
});

test("corrupt lines are tolerated; a valid matching line still injects", () => {
  const sb = sandbox({ config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app" } } });
...
test("config present but NO entry matches this repo -> silent", () => {
  const sb = sandbox({
    config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "nonesuch-repo" } },
...
test("agent.memory_inject: 'off' -> silent even with matching entries", () => {
  const sb = sandbox({
    config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app", memory_inject: "off" } },
...
test("agent.memory_inject: 'on' (explicit) with a config present -> injects", () => {
  const sb = sandbox({
    config: { version: 1, agent: { receipts_version: PLUGIN_VERSION,  repo_name: "web-app", memory_inject: "on" } },
